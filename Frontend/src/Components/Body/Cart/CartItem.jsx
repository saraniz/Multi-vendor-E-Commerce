...
import { deleteFromCart,updateCartItem } from "../../../Storage/Cart/cartAction";
...
const CartItem = ({ item, onQuantityChange, onRemove }) => {
  ...
  // Always treat quantity as a number
  const quantity = Number(item.quantity) || 0;

  // Update quantity locally without backend
const handleIncrease = () => onQuantityChange(item.id, quantity + 1);
const handleDecrease = () => {
  if (quantity > 1) onQuantityChange(item.id, quantity - 1);
};


  console.log("CartItem quantity type:", quantity, typeof quantity);
